...
class ArxivDataModule(pl.LightningDataModule):
    def __init__(self, batch=16, max_length=256):
     ...
     #   )
        self.data_collator = DataCollatorWithPadding(self.tokenizer, max_length=self.max_length, padding='max_length')
    ...
    def tokenize_function(self, samples):
        return self.tokenizer(samples['abstract'], add_special_tokens=True, max_length=self.max_length, padding=True, truncation=True)
